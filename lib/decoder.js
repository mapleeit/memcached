    ...
    this.parsers = {
      ...
      'VALUE'(tokens, dataSet, err, queue) {
        var key = tokens[1]
          ...
          , cas = tokens[4]
          , multi = this.S.queries[0] && this.S.queries[0].multi || cas
            ? {}
            : false
          , tmp;
    ...
    while(this.pipeArray.length && this.allCommands.test(this.pipeArray[0])) {
      ...
      if (this.parsers[tokenSet[0]]) {
        ...
        switch (resultSet.shift()) {
          ...
          case FLUSH:
            const type = S.queries.length > 0 ? S.queries[0].type : undefined
            resultSet = queue;
            ...
            this.emit('step', {
              ...
              result: this.resultParsers[type]
                ? this.resultParsers[type].call(this, resultSet, err)
